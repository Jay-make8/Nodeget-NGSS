          ...
          <div className="space-y-4">
            ...
            >
              <div className="space-y-2">
                <ResourceMini
                  data={filteredHistory} dataKey="cpu" label="CPU" color={C_CPU} currentVal={u.cpu}
                  sub={d?.load_one != null ? `LA ${d.load_one.toFixed(2)}` : (cpu?.physical_cores != null ? `${cpu.physical_cores} vCPU` : undefined)}
                  detail={cpu?.brand ?? cpu?.per_core?.[0]?.brand}
                />
                <ResourceMini data={filteredHistory} dataKey="mem"  label="内存"  color={C_MEM}  currentVal={u.mem}  sub={u.memTotal ? `${bytes(u.memUsed)} / ${bytes(u.memTotal)}` : undefined} />
                <ResourceMini data={filteredHistory} dataKey="disk" label="磁盘"  color={C_DISK} currentVal={u.disk} sub={u.diskTotal ? `${bytes(u.diskUsed)} / ${bytes(u.diskTotal)}` : undefined} />
              </div>
            </Panel>
